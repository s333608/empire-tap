...
export default function Home() {
  ...
  // Keep refs in sync so the sync() closure always has fresh values
  ...
  useEffect(() => {
    ...
    async function boot() {
      // In local dev (no Telegram) raw is empty — show dev mode
      if (!raw) {
        setError('dev');
        return;
      }
      ...
      try {
        ...
        if (!res.ok) { setError('auth'); return; }
  ...
  // ── Energy regen (1 per 2 sec, max 1000) ─────────────────────────────
  useEffect(() => {
  ...
  function tap(e: React.MouseEvent<HTMLDivElement>) {
    ...
    setFloats(f => [...f, { id, x, y, value: earned }]);
    ...
    return (
      <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center gap-4 p-8">
        ...
        <p className="text-gray-400 text-center max-w-xs">
          This game runs inside Telegram. Open it via your bot to play for real.
        </p>
        <p className="text-xs text-gray-600 mt-4">
          Deploy → BotFather → set Mini App URL → open bot
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center p-6 select-none overflow-hidden">
      ...
      <div className="mt-6 flex gap-6 text-center text-sm text-gray-400">
        ...
        <div>
          <div className="text-white font-bold text-lg">{autoPerSec}/s</div>
          <div>Auto-earn</div>
        </div>
      </div>
